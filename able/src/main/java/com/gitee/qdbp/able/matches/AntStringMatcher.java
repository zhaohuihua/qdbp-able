...
public class AntStringMatcher implements StringMatcher {
    ...
    protected boolean doMatch(String pattern, String path, boolean fullMatch) {
        if (startsWithSeparator(path) != startsWithSeparator(pattern)) {
            return false;
        }

        String[] pattDirs = StringTools.split(pattern, this.pathSeparator);
        String[] pathDirs = StringTools.split(path, this.pathSeparator);
        ...
        if (pathIdxStart > pathIdxEnd) {
            ...
            if (pattIdxStart > pattIdxEnd) {
                return (endsWithSeparator(pattern) ? endsWithSeparator(path) : !endsWithSeparator(path));
            }
            if (!fullMatch) {
                return true;
            }
            if (pattIdxStart == pattIdxEnd && pattDirs[pattIdxStart].equals("*") && endsWithSeparator(path)) {
                return true;
            }
    ...
    public String extractPathWithinPattern(String pattern, String path) {
        String[] patternParts = StringTools.split(pattern, this.pathSeparator);
        String[] pathParts = StringTools.split(path, this.pathSeparator);
        ...
        for (int i = 0; i < patternParts.length; i++) {
            ...
            if ((patternPart.indexOf('*') > -1 || patternPart.indexOf('?') > -1) && pathParts.length >= i + 1) {
                if (puts > 0 || (i == 0 && !startsWithSeparator(pattern))) {
    ...
    private boolean endsWithSeparator(String string) {
        ...
        return lastChar == this.pathSeparator;
    }

    private boolean startsWithSeparator(String string) {
        if (string == null || string.length() == 0) return false;
